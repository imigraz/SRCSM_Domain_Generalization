...
from tensorflow_train.utils.tensorflow_util import reduce_mean_masked, masked_bit, save_reduce_mean, most_significant_bit, reduce_median_masked, reduce_mean_weighted


def cosine_embedding_single_instance_loss(embeddings, target_instances_mask, other_instances_mask, normalized_embeddings=None, term_1_2_normalization='individual', term_0_squared=False, term_1_squared=False, return_h_norm=False, is_background=False, data_format='channels_first', weights=None):
    if data_format == 'channels_first':
        ...
        if embeddings.shape.ndims == 3:
            ...
            image_axes = [0, 1, 2]
        assert 'channels_last is currently not supported and tested'
    ...
    if not is_background:
        ...
        #h = reduce_median_masked(tf.transpose(embeddings, [1, 2, 3, 0]), target_instances_mask[0, ...], axis=0, keepdims=False)
        #h = tf.expand_dims(tf.expand_dims(tf.expand_dims(h, axis=1), axis=2), axis=3)
        #h = reduce_median_masked(embeddings, tf.broadcast_to(target_instances_mask, embeddings.shape), axis=0, keepdims=True)
        #print(h.shape)
    else:
        if embeddings.shape.ndims == 3:
            if term_1_squared:
    ...
    h_norm = tf.nn.l2_normalize(h, axis=embedding_axis)
    # l2_normalize embeddings -> needed for cos_simliarity
    if normalized_embeddings is None:
        normalized_embeddings = tf.nn.l2_normalize(embeddings, axis=embedding_axis)
    # calculate cos_similarity with target mean embedding and all embeddings
    cos_similarity = tf.reduce_sum(h_norm * normalized_embeddings, axis=embedding_axis, keepdims=True)
    dot_product = tf.reduce_sum(h * embeddings, axis=embedding_axis, keepdims=True)
    ...
    if term_1_squared:
        ...
        #alpha = 0.5
        #isrlu = lambda x: tf.where(x >= 0, x, x / (tf.sqrt(1 + 1/(alpha*alpha) * x^2))) + alpha
        #term_1 = isrlu(cos_similarity - 0.5)
        term_1 = tf.nn.relu(cos_similarity - 0.75)
        #tf.nn.elu
        #term_1 = tf.nn.relu(cos_similarity - 0.5)
        #term_1 = tf.pow(tf.nn.relu(cos_similarity), 2)
        #term_1 = tf.nn.leaky_relu(cos_similarity - 0.5, alpha=0.01)
    ...
    if term_1_2_normalization == 'individual':
        if weights is None:
            ...
            target_instances_weights = target_instances_mask * weights
            other_instances_weights = other_instances_mask * weights
            term_0 = tf.expand_dims(reduce_mean_weighted(term_0, target_instances_weights), axis=0)
            term_1 = tf.expand_dims(reduce_mean_weighted(term_1, other_instances_weights), axis=0)
        return term_0, term_1
    elif term_1_2_normalization == 'none' or term_1_2_normalization == 'combined':
        assert weights is not None, 'weights are only supportet for \'individual\' normalization'
...
def cosine_embedding_all_instances_loss(embeddings, instances, normalized_embeddings, term_1_2_normalization, return_h_norm, parallel_iterations=1, data_format='channels_first', *args, **kwargs):
    ...
    def per_instance_loss(i, current_ta_term_0, current_ta_term_1, current_ta_term_2):
        ...
        current_terms = tf.cond(tf.greater(tf.count_nonzero(target_instances_mask), 0),
                                lambda: cosine_embedding_single_instance_loss(embeddings, target_instances_mask, other_instances_mask, normalized_embeddings, term_1_2_normalization=term_1_2_normalization, return_h_norm=return_h_norm, data_format=data_format, *args, **kwargs),
...
def cosine_embedding_all_instances_bitwise_loss(embeddings, instances_bitwise, normalized_embeddings, term_1_2_normalization, return_h_norm, parallel_iterations=1, data_format='channels_first', *args, **kwargs):
...
def cosine_embedding_per_instance_loss(embeddings, instances, normalized_embeddings=True, bitwise_instances=False, term_1_2_normalization='individual', l=1.0, *args, **kwargs):
    if not normalized_embeddings:
        embeddings_l2_normalized = tf.nn.l2_normalize(embeddings, axis=0)
    else:
        embeddings_l2_normalized = embeddings
    with tf.name_scope('cosine_embedding_loss'):
        if not bitwise_instances:
            terms = cosine_embedding_all_instances_loss(embeddings, instances, embeddings_l2_normalized, term_1_2_normalization, *args, **kwargs)
        else:
            terms = cosine_embedding_all_instances_bitwise_loss(embeddings, instances, embeddings_l2_normalized, term_1_2_normalization, *args, **kwargs)
...
def cosine_embedding_per_instance_batch_loss(embeddings, instances, l=1.0, l2=0.0, *args, **kwargs):
    loss_list_term_0 = []
    loss_list_term_1 = []
    loss_list_term_2 = []
    return_h_norm = l2 > 0.0
    for i in range(embeddings.get_shape().as_list()[0]):
        current_loss = cosine_embedding_per_instance_loss(embeddings[i, ...], instances[i, ...], l=l, return_h_norm=return_h_norm, *args, **kwargs)
