...
class MainLoopBase(object):
    ...
    def load_model(self, model_filename=None, assert_consumed=False):
        ...
        # checkpoint_reader = tf.train.load_checkpoint(model_filename)
        # shape_from_key = checkpoint_reader.get_variable_to_shape_map()
        # dtype_from_key = checkpoint_reader.get_variable_to_dtype_map()
        # tensor_dict = {x: checkpoint_reader.get_tensor(x) for x in shape_from_key.keys()}
        ...
        while self.current_iter <= self.max_iter:
            ...
            if self.current_iter < self.max_iter:
                ...
                if self.loss_metric_logger_train is not None and (self.current_iter % self.disp_iter) == 0:
                    if self.optimizer:
                        summary_values = self.loss_metric_logger_train.finalize_with_optimizer(self.optimizer)
                    else:
            ...
            if tf.is_tensor(self.current_iter):
                self.current_iter.assign_add(1)  # defined as tf.Variable to be accessible in tf.function
            else:
                self.current_iter += 1
    ...
    def run_test_model_list(self, load_model_params_dict_list):
        """
        Init all, load model and run test loop.
        """
        try:
            ...
            if self.output_folder_handler is not None:
                print('Output folder:', self.output_folder_handler.folder_base())
            for cur_load_model_dict in load_model_params_dict_list:
                load_model_filename = cur_load_model_dict['load_model_filename']
                load_model_iter = cur_load_model_dict['load_model_iter']
                self.current_iter = load_model_iter
                self.load_model(model_filename=load_model_filename)
                self.test()
        finally:
            self.close_all()
    ...
    def run_generate_training_images(self, num_images):
        ...
        try:
            ...
            for i in range(num_images):
                entry = self.dataset_train_1.get_next()
                if 'id' not in entry:
                    entry['id'] = {'image_id': f'image_{i}'}
                self.save_dataset_entry(entry, 'training_images')
        finally:
            self.close_all()

    def run_generate_validation_images(self):
        """
        Init dataset and generate validation images.
        """
        try:
            self.init_datasets()
            for i in range(self.dataset_val.num_images()):
                entry = self.dataset_val.get_next()
                self.save_dataset_entry(entry, 'validation_images')
        finally:
            self.close_all()

    def save_dataset_entry(self, dataset_entry, path):
        """
        Write dataset entry to path.
        :param dataset_entry: The dataset entry.
        :param path: The path to write the files to.
        """
        image_id = dataset_entry['id']['image_id']
        for key, value in dataset_entry['generators'].items():
            if len(value.shape) in [3, 4]:
                write_multichannel_np(value, os.path.join(path, f'{image_id}_{key}.nii.gz'), data_format=self.data_format, image_type=np.float32)
            elif len(value.shape) in [0, 1]:
                save_string_txt(str(value), os.path.join(path, f'{image_id}_{key}.txt'))
    ...
    def init_output_folder_handler(self):
        ...
        """
        if self.model:
            self.output_folder_handler = OutputFolderHandler(base_folder='./output', model_name=self.model.name)
    ...
    def init_checkpoint_manager(self):
        ...
        """
        if self.checkpoint and self.output_folder_handler:
