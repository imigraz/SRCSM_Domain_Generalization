...
class Mixer(tf.keras.layers.Layer):
    ...
    def build(self, training):
        self.fcn = Conv3D(self.num_filters_base, 1, name=self.nameio + '_fcn', activation=self.activation, data_format=self.data_format, kernel_initializer=self.kernel_initializer, kernel_regularizer=l2(1.0), padding=self.padding)
        self.conv = Conv3D(self.num_filters_base // self.reduce_factor2, self.kernel_size, name=self.nameio + '_conv', activation=self.activation, data_format=self.data_format, kernel_initializer=self.kernel_initializer, kernel_regularizer=l2(1.0), padding=self.padding)
        self.up = Conv3D(self.num_filters_base // self.reduce_factor, 1, name=self.nameio + '_up', activation=self.activation, data_format=self.data_format, kernel_initializer=self.kernel_initializer, kernel_regularizer=l2(1.0), padding=self.padding)
        self.down = Conv3D(self.num_filters_base // self.reduce_factor, 1, name=self.nameio + '_down', activation=self.activation, data_format=self.data_format, kernel_initializer=self.kernel_initializer, kernel_regularizer=l2(1.0), padding=self.padding)
        self.left = Conv3D(self.num_filters_base // self.reduce_factor, 1, name=self.nameio + '_left', activation=self.activation, data_format=self.data_format, kernel_initializer=self.kernel_initializer, kernel_regularizer=l2(1.0), padding=self.padding)
        self.right = Conv3D(self.num_filters_base // self.reduce_factor, 1, name=self.nameio + '_right', activation=self.activation, data_format=self.data_format, kernel_initializer=self.kernel_initializer, kernel_regularizer=l2(1.0), padding=self.padding)
        self.front = Conv3D(self.num_filters_base // self.reduce_factor, 1, name=self.nameio + '_front', activation=self.activation, data_format=self.data_format, kernel_initializer=self.kernel_initializer, kernel_regularizer=l2(1.0), padding=self.padding)
        self.back = Conv3D(self.num_filters_base // self.reduce_factor, 1, name=self.nameio + '_back', activation=self.activation, data_format=self.data_format, kernel_initializer=self.kernel_initializer, kernel_regularizer=l2(1.0), padding=self.padding)
    ...
    def call(self, x, **kwargs):
        ...
        left = self.directional(left, axis=-1)
        right = self.directional(right, axis=-1, reverse=True)
        up = self.directional(up, axis=-2)
        down = self.directional(down, axis=-2, reverse=True)
        front = self.directional(front, axis=-3)
        back = self.directional(back, axis=-3, reverse=True)

        x = tf.concat((conv, left, right, up, down, front, back), axis=1)
