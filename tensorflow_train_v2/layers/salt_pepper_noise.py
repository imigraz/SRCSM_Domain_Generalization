...
def salt_pepper_3D(image, rate, scales=[1], data_format='channels_first', seed=None):
    ...
    for scale in scales:
        ...
        # scale_to_use = [scale if x % scale == 0 else 1 for x in shape]
...
def salt_pepper_3D_anisotropic(image, rate, scales=[[1] * 3], data_format='channels_first', seed=None):
    image = tf.convert_to_tensor(image, name="image")
    rate = tf.convert_to_tensor(rate, dtype=image.dtype, name="rate")
    ...
    for scale in scales:
        ...
        pooling_layer = AveragePooling3D(scale, data_format=data_format)
        upsampling_layer = UpSampling3DLinear(scale, data_format=data_format)
