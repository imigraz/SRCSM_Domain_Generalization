...
class DebugImageDataset(DatasetBase):
    ...
    def get_debug_image(self, image):
        ...
        if self.debug_image_type == 'default':
        ...
        elif self.debug_image_type == 'projection':
            if len(image.shape) == 3:
                image_1 = np.amax(image, axis=0)
                image_2 = np.amax(image, axis=1)
                image_3 = np.amax(image, axis=2)
                image = np.concatenate((image_1, image_2, image_3), axis=1)

            if len(image.shape) == 4:
                image_1 = np.amax(image[0, :, :, :], axis=0)
                image_2 = np.amax(image[0, :, :, :], axis=1)
                image_3 = np.amax(image[0, :, :, :], axis=2)
                image = np.concatenate((image_1, image_2, image_3), axis=1)
            return image

        elif self.debug_image_type == 'middle_image':
            pos = int(image[0].shape[0] / 2)
            if len(image.shape) == 3:
                image_1 = image[0, pos, :, :]
                image_2 = image[0, :, pos, :]
                image_3 = image[0, :, :, pos]
            ...
            if len(image.shape) == 4:
                ...
                image_2 = image[-1, :, pos, :]
                image_3 = image[-1, :, :, pos]
                image_a = np.concatenate((image_1, image_2, image_3), axis=1)
                image_1 = image[1, pos, :, :]
                image_2 = image[1, :, pos, :]
                image_3 = image[1, :, :, pos]
                image_b = np.concatenate((image_1, image_2, image_3), axis=1)
    ...
    def save_debug_image(self, image, file_name):
        ...
        """
        if (self.debug_image_type == "middle_image" or self.debug_image_type == 'projection'):
