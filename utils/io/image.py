...
from utils.sitk_image import label_to_rgb, set_spacing_origin_direction, resample
...
def read(path, sitk_pixel_type=sitk.sitkInt16):
    """
    Read an sitk image from a given file path.
    :param path: The path of the file.
    :param sitk_pixel_type: The sitk pixel type.
...
def create_channel_layout_image(image, mode, data_format, label_to_rgb_function=label_to_rgb, *args, **kwargs):
    """
    ...
    :param label_to_rgb_function: Use this function to convert label indizes to RGB colors.
    ...
    if mode == 'label_rgb':
        ...
        # label_map = np.stack([label_to_rgb_function(i) for i in range(max_value + 1)]).astype(np.float32)
        # rgb_image = label_map[current_image]
        labels = np.unique(current_image)
        rgb_image = np.zeros(current_image.shape + (3,), dtype=np.float32)
        for label in labels:
            if label == 0:
                continue
            rgb_image[current_image == label] = label_to_rgb_function(label)
        if data_format == 'channels_first':
            rgb_image = np.moveaxis(rgb_image, 0, -1)
        return rgb_image
    if mode == 'channel_rgb' or mode == 'channel_rgb_no_first':
        rgb_image = None
        for i, current_image in enumerate(np.moveaxis(image, channel_axis, 0)):
            if mode == 'channel_rgb_no_first' and i == 0:
                continue
            if rgb_image is None:
                rgb_image = np.zeros(current_image.shape + (3,), dtype=np.float32)
            current_rgb_image = np.expand_dims(current_image, axis=-1) * label_to_rgb_function(i)
            rgb_image = np.maximum(rgb_image, current_rgb_image)
        if data_format == 'channels_first':
            rgb_image = np.moveaxis(rgb_image, 0, -1)
        return rgb_image
...
def create_image_layout_image(image, mode, data_format, *args, **kwargs):
    """
    Convert the np input image to a given layout.
    :param image: The np image.
    :param mode: One of the following:
    ...
    if mode == 'max_projection' or mode == 'avg_projection' or mode == 'center_slice_projection':
        ...
        projections = [projection_function(current_image) for current_image in np.moveaxis(image, channel_axis, 0)]
        return np.stack(projections, axis=channel_axis)
    raise ValueError('Unsupported layout mode: ' + mode)


def normalize_image_to_np_range(image, mode, dtype, *args, **kwargs):
    """
...
def create_sitk_image(image, mode, data_format, spacing=None, origin=None, direction=None, *args, **kwargs):
    """
    ...
    if mode == 'vector':
        ...
        sitk_image = sitk.GetImageFromArray(image, isVector=True)
        set_spacing_origin_direction(sitk_image, spacing, origin, direction)
        return sitk_image
    if mode == 'split':
        image_list = []
        for current_image in np.moveaxis(image, channel_axis, 0):
            sitk_image = sitk.GetImageFromArray(current_image, isVector=False)
            set_spacing_origin_direction(sitk_image, spacing, origin, direction)
            image_list.append(sitk_image)
        return image_list
    if mode == 'gallery':
        return sitk.GetImageFromArray(utils.np_image.gallery(list(np.moveaxis(image, channel_axis, 0))), isVector=False)
...
def write_multichannel_np(image,
    ...
    image = create_channel_layout_image(image, channel_layout_mode, data_format, *args, **kwargs)
    image = create_image_layout_image(image, image_layout_mode, data_format, *args, **kwargs)
    image = normalize_image_to_np_range(image, output_normalization_mode, image_type, *args, **kwargs)
    image = image.astype(image_type)
    image_sitk = create_sitk_image(image, sitk_image_output_mode, data_format, spacing, origin, direction, *args, **kwargs)
    if 'output_resize_factor' in kwargs and float(kwargs['output_resize_factor']) != 1.0:
        s = sitk.AffineTransform(image_sitk.GetDimension())
        s.Scale(float(kwargs['output_resize_factor']))
        output_size = [int(o) for o in np.array(image_sitk.GetSize()) / float(kwargs['output_resize_factor'])]
        image_sitk = resample(image_sitk, transform=s, output_size=output_size, output_spacing=None, output_origin=None, output_direction=None, interpolator='nearest', output_pixel_type=None, default_pixel_value=None)
...
def write_multichannel_sitk(image, *args, **kwargs):
    ...
    if not 'spacing' in kwargs:
        kwargs['spacing'] = image.GetSpacing()
    if not 'origin' in kwargs:
        kwargs['origin'] = image.GetOrigin()
    if not 'direction' in kwargs:
        kwargs['direction'] = image.GetDirection()
    write_multichannel_np(image_np, *args, **kwargs)
