...
def resample_to_spacing(image, new_spacing, interpolator=None):
    ...
    if np.allclose(old_spacing, new_spacing):
        return image
...
def transform_np_output_to_input(output_image, output_spacing, channel_axis, input_image_size, input_image_spacing, input_image_origin, input_image_direction, transform, interpolator='linear', output_pixel_type=None):
    ...
    #         print('submit')
    #         future = executor.submit(transform_single_np_output_to_input,
    #                                  x,
    #                                  output_spacing,
    #                                  input_image_size,
    #                                  input_image_spacing,
...
def apply_np_image_function_image_list(image_list, f):
    image_np_list = [utils.sitk_np.sitk_to_np(x) for x in image_list]
    image_np_list = f(image_np_list)
    output_list = [utils.sitk_np.np_to_sitk(x) for x in image_np_list]
    for image, output in zip(image_list, output_list):
        copy_information(image, output)
    return output_list
...
def get_segmentation_boundary(label_sitk, boundary_size):
    label_np = utils.sitk_np.sitk_to_np(label_sitk)
    ...
    if len(label_np.shape) == 2:
        ...
        # default will be box kernel
        kernel = None

    boundary_np = np.zeros_like(label_np)
    for cur_label in unique:
        if cur_label == 0:
            continue

        cur_binary_fg = np.where(label_np == cur_label, 1, 0).astype(np.uint8)
        cur_binary_bg = 1 - cur_binary_fg
        cur_binary_bg_dilated = scipy.ndimage.binary_dilation(cur_binary_bg, structure=kernel, iterations=boundary_size)
        cur_boundary = cur_binary_fg * cur_binary_bg_dilated
        boundary_np = np.where(cur_boundary == 1, cur_label, boundary_np)

    boundary_label_sitk = utils.sitk_np.np_to_sitk(boundary_np)

    # sitk.WriteImage(label_sitk, 'DEBUG_label_sitk.nrrd', useCompression=True)
    # sitk.WriteImage(boundary_label_sitk, 'DEBUG_boundary_label_sitk.nrrd', useCompression=True)

    return boundary_label_sitk
