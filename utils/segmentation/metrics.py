...
class TpFpFnMetricPerLabel(TpFpFnMetricBase):
    ...
    """
    def __init__(self, boundary_size=None):
        super(TpFpFnMetricPerLabel, self).__init__()
    ...
    def __call__(self, prediction_sitk, groundtruth_sitk, labels):
        ...
        if self.boundary_size is not None:
            prediction_sitk = utils.sitk_image.get_segmentation_boundary(prediction_sitk, self.boundary_size)
            groundtruth_sitk = utils.sitk_image.get_segmentation_boundary(groundtruth_sitk, self.boundary_size)
...
class BoundaryDiceMetric(TpFpFnMetricPerLabel):
    """
    Boundary Dice metric.
    """

    def __init__(self, boundary_size):
        super(BoundaryDiceMetric, self).__init__(boundary_size)
