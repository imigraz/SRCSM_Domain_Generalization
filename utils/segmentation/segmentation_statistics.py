...
class SegmentationStatistics(object):
    def __init__(self,
        ...
        if self.ignore_label is not None and self.ignore_label in self.labels:
            print('SegmentationStatistics warning: ignore_label is in list of labels')
    ...
    def add_labels(self, current_id, prediction_labels, groundtruth_labels):
        ...
        self.set_metric_values(current_id, current_metric_values)
    ...
    def get_metric_mean_dict(self, metric_key, calculate_overall_mean=True):
        metric_mean_list = self.get_metric_mean_list(metric_key)
        mean_dict = {}
        if calculate_overall_mean and len(metric_mean_list) > 1:
            metric_mean_all = np.mean(metric_mean_list)
            mean_dict[f'mean_{metric_key}'] = metric_mean_all
        for l, m in zip(self.labels, metric_mean_list):
            mean_dict[f'{metric_key}_{l}'] = m
        return mean_dict
    ...
    def finalize(self, file_name_suffix=''):
        for metric_key in self.metrics.keys():
            if isinstance(metric_key, tuple):
                for current_metric_key in metric_key:
                    self.save_metric_values(current_metric_key, file_name_suffix)
            else:
                self.save_metric_values(metric_key, file_name_suffix)
    ...
    def get_metric_values(self, predictions_sitk, groundtruth_sitk):
        if self.ignore_label is not None:
            ignored_sitk = groundtruth_sitk == self.ignore_label
            not_ignored_sitk = groundtruth_sitk != self.ignore_label
            predictions_sitk = predictions_sitk * not_ignored_sitk + groundtruth_sitk * ignored_sitk
    ...
    def save_metric_values(self, metric_key, file_name_suffix=''):
        ...
        if file_name_suffix and len(file_name_suffix) > 0 and not file_name_suffix.startswith('_'):
            file_name_suffix = '_' + file_name_suffix
        utils.io.text.save_dict_csv(metric_dict, os.path.join(self.output_folder, metric_key + f'{file_name_suffix}.csv'), header)
