...
def draw_ellipse(image, center, axes, angle, value=1):
    y, x = np.meshgrid(range(image.shape[0]), range(image.shape[1]), indexing='ij')
    a =(np.cos(angle)*(center[1]-x)+np.sin(angle)*(center[0]-y)) ** 2
    b =(np.sin(angle)*(center[1]-x)-np.cos(angle)*(center[0]-y)) ** 2
    d = (a / (axes[1] / 2) ** 2) + (b / (axes[0] / 2) ** 2)
    image[d <= 1] = value
    return image
