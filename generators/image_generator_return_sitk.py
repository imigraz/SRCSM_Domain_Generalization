...
class ImageGeneratorReturnSITK(TransformationGeneratorBase):
    """
    ...
    def __init__(self,
        ...
        """
        super(ImageGeneratorReturnSITK, self).__init__(dim=dim,
                                             *args, **kwargs)
    ...
    def get(self, image, transformation, **kwargs):
        ...
        final_output_image_sitk = sitk.GetImageFromArray(np.squeeze(output_image_np).astype(np.float32))
        if output_image_sitk:
            final_output_image_sitk.CopyInformation(output_image_sitk)

        return final_output_image_sitk
